...
class InsResearchAPIError(Exception):
    """API错误异常类"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="同花顺机构研究与评级数据查询工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python3 scripts/cli.py --query "同花顺研报评级"
  python3 scripts/cli.py --query "业绩预测" --page "1" --limit "20"
  python3 scripts/cli.py --query "券商金股" --api-key "your-key"
...
def query_insresearch_data(query: str, page: str, limit: str,
    ...
    Raises:
        InsResearchAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise InsResearchAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise InsResearchAPIError(f"API返回错误: {status_msg}")
        ...
        raise InsResearchAPIError(
            f"HTTP错误 {e.code}: {e.reason}",
            status_code=e.code,
            response=error_body
        )
    except urllib.error.URLError as e:
        raise InsResearchAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise InsResearchAPIError(f"响应解析失败: {e}")
...
def main():
    ...
    except InsResearchAPIError as e:
        error_output = {
            "success": False,
            "error": e.message
        }
