---
name: 机构研究与评级查询
description: 同花顺机构研究与评级查询skill。查询研报评级、业绩预测、ESG、信用评级、主体评级、基金评级、券商金股等机构观点数据，支持自然语言问句输入，返回相关机构研究数据结果。当用户询问研报评级、业绩预测、ESG评级、信用评级、主体评级、基金评级、券商金股等机构研究数据查询问题时，必须使用此技能。
---
...
- **多次查询**：如果用户问题涉及多个维度（如"研报评级和业绩预测"），需要拆分为多个标准 query 分别调用接口
...
# 查询业绩预测
python3 scripts/cli.py --query "业绩预测"

# 查询券商金股
python3 scripts/cli.py --query "券商金股"

# 翻页查询
python3 scripts/cli.py --query "研报评级" --page 2 --limit 20
```
