...
class HKStockSelectorAPIError(Exception):
    """API错误异常类"""
...
def query_hkstock(query: str, page: str, limit: str,
    ...
    Raises:
        HKStockSelectorAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise HKStockSelectorAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if "status_code" in result and result["status_code"] != 0:
                    ...
                    raise HKStockSelectorAPIError(f"API返回错误: {message}")
        ...
        raise HKStockSelectorAPIError(
            f"HTTP错误 {e.code}: {e.reason}",
            status_code=e.code,
            response=error_body
        )
    except urllib.error.URLError as e:
        raise HKStockSelectorAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise HKStockSelectorAPIError(f"响应解析失败: {e}")
...
def main():
    ...
    try:
        ...
        if code_count > len(datas):
            output["pagination_tip"] = f"共查到 {code_count} 只港股，当前返回第 {args.page} 页的 {len(datas)} 条。如需更多数据，请使用 --page 参数翻页。"

        print(json.dumps(output, ensure_ascii=False, indent=2))

    except HKStockSelectorAPIError as e:
        error_output = {
            "success": False,
            "error": e.message
        }
