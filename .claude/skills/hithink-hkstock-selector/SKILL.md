...
**改写规则：**
- 保留用户核心意图（如：港股科技股、涨跌幅超过5%等）
- 将口语化表达转为标准金融术语（如"给我选一个港股" → "港股有哪些"）
- 适当简化过于复杂的复合条件
- 改写后需保持原意不变
...
调用金融查询接口获取数据，支持分页参数。注意：默认返回10条数据，但符合条件的港股总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
# 解析返回数据
datas = result.get("datas", [])           # 当前页港股列表
code_count = result.get("code_count", 0)  # 符合条件的总港股数
...
```python
for item in datas:
    # 根据查询类型提取相应字段，如港股代码、港股简称、涨跌幅等
```

### 步骤 6: 数据扩展决策

skill 需要自行决策当前数据是否足够回答用户问题：
- 如果数据完整：直接返回格式化后的结果且保证选港股表格正确解析为表格展示
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含港股代码、港股简称、涨跌幅等字段 |
...
{
  "datas": [
    {"港股代码": "00700.HK", "港股简称": "腾讯控股", "涨跌幅": 2.35},
    {"港股代码": "09988.HK", "港股简称": "阿里巴巴", "涨跌幅": 1.85}
  ],
  "code_count": 100,
  "chunks_info": {
    "query": "港股科技股",
...
- `code_count` 表示符合条件的总港股数，可能远大于 `datas` 的长度
- 当 `code_count > len(datas)` 时，需要通过 `page` 参数翻页获取更多数据
- 返回的表格数据需要解析 `datas` 数组中的对象字段，如 `港股代码`、`港股简称`、`涨跌幅` 等
...
```bash
# 直接查询（默认返回10条，注意：符合条件的港股总数可能更多）
python3 scripts/cli.py --query "港股科技股有哪些？"

# 指定分页参数（page从1开始，limit为每页条数）
python3 scripts/cli.py --query "港股银行股" --page "1" --limit "20"

# 指定API密钥
python3 scripts/cli.py --query "北向资金增持的港股" --api-key "your-key"
```

**分页说明：**
- 默认返回10条数据，但符合条件的港股总数（code_count）可能远大于10
