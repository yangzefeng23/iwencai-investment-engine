---
name: 宏观数据查询
description: 同花顺宏观数据查询skill。查询 GDP、CPI、PPI、利率、汇率、社融等宏观经济指标，支持自然语言问句输入，返回相关宏观经济数据结果。当用户询问宏观经济数据、GDP、CPI、PPI、利率、汇率、社融、M2、PMI、工业增加值、消费、投资、进出口等宏观经济指标查询问题时，必须使用此技能。
---
...
本技能提供宏观数据查询能力，支持：
- GDP、CPI、PPI 等国民经济核算指标
- 利率、汇率、社融等金融指标
- PMI、工业增加值、消费、投资、进出口等经济运行指标
- 支持自然语言问句输入，返回相关宏观经济数据结果
...
- **多次查询**：如果用户问题涉及多个指标（如"GDP和CPI数据"），需要拆分为多个标准 query 分别调用接口
...
# 查询LPR利率
python3 scripts/cli.py --query "LPR利率"

# 翻页查询
python3 scripts/cli.py --query "GDP数据" --page 2 --limit 20
```
