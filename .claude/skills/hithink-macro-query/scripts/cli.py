...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="同花顺宏观经济数据查询工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python3 scripts/cli.py --query "2024年中国GDP"
  python3 scripts/cli.py --query "最近一期CPI" --page "1" --limit "20"
  python3 scripts/cli.py --query "LPR利率" --api-key "your-key"
...
def query_macro_data(query: str, page: str, limit: str,
    ...
    Raises:
        MacroDataAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise MacroDataAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise MacroDataAPIError(f"API返回错误: {status_msg}")
        ...
        raise MacroDataAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise MacroDataAPIError(f"响应解析失败: {e}")
