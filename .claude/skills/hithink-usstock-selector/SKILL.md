---
name: 问财选美股
description: 同花顺智能选美股skill。通过自然语言查询进行美股筛选，支持行情指标、财务指标、行业概念、业绩预测、研报评级等多条件组合筛选。返回符合条件的相关美股数据。当用户询问美股筛选问题时，必须使用此技能。
---
...
本技能提供美股股票智能筛选能力，通过自然语言查询支持：
- 行情指标筛选（股价、涨跌幅、成交量等）
- 财务指标筛选（市盈率、市净率、营收、利润等）
- 行业概念筛选（科技、医药、消费、金融等）
- 业绩预测筛选（盈利预测、增长率预测等）
- 研报评级筛选（买入、增持、中性、卖出等）
- 多条件组合筛选
...
**改写规则：**
- 保留用户核心意图（如：涨幅前五、评级买入等）
- 将口语化表达转为标准金融术语（如"帮我选涨得好的美股" → "涨幅前五的美股"）
- 适当简化过于复杂的复合条件
- 改写后需保持原意不变
...
调用金融查询接口获取数据，支持分页参数。注意：默认返回10条数据，但符合条件的美股总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
# 解析返回数据
datas = result.get("datas", [])           # 当前页美股列表
code_count = result.get("code_count", 0)  # 符合条件的总美股数
...
```python
for item in datas:
    # 根据查询类型提取相应字段，如股票名称、涨跌幅、市盈率、评级等
```

### 步骤 6: 数据扩展决策

skill 需要自行决策当前数据是否足够回答用户问题：
- 如果数据完整：直接返回格式化后的结果且保证选美股表格正确解析为表格展示
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含股票名称、涨跌幅、市盈率、评级等字段 |
| code_count | INT | 符合查询条件的总美股数量（注意：可能大于当前返回的datas条数） |
...
{
  "datas": [
    {"股票名称": "苹果", "涨跌幅": 2.5, "市盈率": 25.3, "评级": "买入"},
    {"股票名称": "微软", "涨跌幅": 1.8, "市盈率": 30.1, "评级": "增持"}
  ],
  "code_count": 80,
  "chunks_info": {
    "query": "评级买入的美股",
    "parsed_conditions": ["评级=买入"]
  }
}
```
...
- `code_count` 表示符合条件的总美股数，可能远大于 `datas` 的长度
- 当 `code_count > len(datas)` 时，需要通过 `page` 参数翻页获取更多数据
- 返回的表格数据需要解析 `datas` 数组中的对象字段，如 `股票名称`、`涨跌幅`、`市盈率`、`评级` 等
...
```bash
# 直接查询（默认返回10条，注意：符合条件的美股总数可能更多）
python3 scripts/cli.py --query "美股科技股有哪些？"

# 指定分页参数（page从1开始，limit为每页条数）
python3 scripts/cli.py --query "评级买入的美股" --page "1" --limit "20"

# 指定API密钥
python3 scripts/cli.py --query "美股市盈率低于20" --api-key "your-key"
```

**分页说明：**
- 默认返回10条数据，但符合条件的美股总数（code_count）可能远大于10
- 当 code_count > 返回条数时，表示还有更多数据，可通过 `--page` 参数翻页获取
- 例如：code_count=80，limit=10，则需要翻8页才能获取全部数据
