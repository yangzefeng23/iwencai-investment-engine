---
name: 期货期权数据查询
description: 同花顺期货期权数据查询skill。查询期货期权的行情、波动率、产销、会员持仓、会员榜单、行权等多种数据，支持自然语言问句输入，返回相关期货期权数据结果。当用户询问期货期权的行情、波动率、产销、会员持仓、会员榜单、行权等问题时，必须使用此技能。
---
...
本技能提供期货期权数据查询能力，支持：
- 期货期权行情数据（价格、涨跌幅、成交量等）
- 波动率数据（隐含波动率、历史波动率等）
- 产销数据（库存、产量、销量等）
- 会员持仓数据（持仓量、持仓变化等）
- 会员榜单数据（成交量排行、持仓量排行等）
- 行权数据（行权价、行权量、行权比率等）
- 支持自然语言问句输入，返回相关期货期权数据结果
...
- **多次查询**：如果用户问题涉及多个维度（如"沪铜期货行情和持仓量"），需要拆分为多个标准 query 分别调用接口
...
# 查询产销数据
python3 scripts/cli.py --query "原油期货库存数据"
