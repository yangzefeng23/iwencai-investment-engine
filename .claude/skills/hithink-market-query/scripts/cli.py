...
def query_market_data(query: str, page: str, limit: str,
    ...
    Raises:
        MarketDataAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise MarketDataAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise MarketDataAPIError(f"API返回错误: {status_msg}")
        ...
        raise MarketDataAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise MarketDataAPIError(f"响应解析失败: {e}")
