...
class ZhishuDataAPIError(Exception):
    """API错误异常类"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="同花顺指数数据查询工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python3 scripts/cli.py --query "上证指数涨跌幅"
  python3 scripts/cli.py --query "沪深300最新点位" --page "1" --limit "20"
  python3 scripts/cli.py --query "创业板指成交量" --api-key "your-key"
...
def query_zhishu_data(query: str, page: str, limit: str,
    ...
    Raises:
        ZhishuDataAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise ZhishuDataAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise ZhishuDataAPIError(f"API返回错误: {status_msg}")
        ...
        raise ZhishuDataAPIError(
            f"HTTP错误 {e.code}: {e.reason}",
            status_code=e.code,
            response=error_body
        )
    except urllib.error.URLError as e:
        raise ZhishuDataAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise ZhishuDataAPIError(f"响应解析失败: {e}")
...
def main():
    ...
    except ZhishuDataAPIError as e:
        error_output = {
            "success": False,
            "error": e.message
        }
